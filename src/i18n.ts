...
const EN: Dict = {
  ...
  'cd.contentHash.witness': 'Hash-only — only the {h}… fingerprint was submitted; the content never leaves the client, so there is nothing to re-hash (proven by the signature + anchor)',
  ...
  'cd.ed25519.pass': 'Signature over the canonical entry verified against {kid} ✓',
  ...
  'cd.opReturnMagic.pass': '{magic} magic ✓ · seq {a}–{b} · root {r}…',
  'cd.opReturnMagic.passLayout': '{magic} magic ✓ · {layout} · root {r}…',
  'cd.opReturnMagic.fail': 'OP_RETURN payload missing LPR anchor magic (QE20/LPR1) or wrong length ({bytes} bytes)',
...
const DE: Dict = {
  ...
  'cd.contentHash.witness': 'Nur-Hash — nur der Fingerabdruck {h}… wurde übermittelt; der Inhalt verlässt den Client nie, es gibt nichts neu zu hashen (bewiesen durch Signatur + Verankerung)',
  ...
  'cd.ed25519.pass': 'Signatur über den kanonischen Eintrag gegen {kid} verifiziert ✓',
  ...
  'cd.opReturnMagic.pass': '{magic}-Magic ✓ · Seq {a}–{b} · Wurzel {r}…',
  'cd.opReturnMagic.passLayout': '{magic}-Magic ✓ · {layout} · Wurzel {r}…',
  'cd.opReturnMagic.fail': 'OP_RETURN-Nutzlast fehlt LPR-Anker-Magic (QE20/LPR1) oder hat falsche Länge ({bytes} Bytes)',
...
const FR: Dict = {
  ...
  'cd.contentHash.witness': "Empreinte seule — seule l'empreinte {h}… a été soumise ; le contenu ne quitte jamais le client, il n'y a rien à recalculer (prouvé par la signature + l'ancrage)",
  ...
  'cd.ed25519.pass': "Signature sur l'entrée canonique vérifiée par rapport à {kid} ✓",
  ...
  'cd.opReturnMagic.pass': 'Magic {magic} ✓ · séq {a}–{b} · racine {r}…',
  'cd.opReturnMagic.passLayout': 'Magic {magic} ✓ · {layout} · racine {r}…',
  'cd.opReturnMagic.fail': 'La charge OP_RETURN n’a pas le magic d’ancrage LPR (QE20/LPR1) ou une longueur incorrecte ({bytes} octets)',
